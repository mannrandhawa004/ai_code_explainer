...
Local development requires MongoDB, Qdrant, and Redis from `docker compose up -d`, plus `OPENAI_API_KEY`. Private indexing additionally requires `GITHUB_APP_ID` and `GITHUB_PRIVATE_KEY`. `INDEXING_CONCURRENCY` is bounded to 1-32. Production requires `REDIS_URL` to use `rediss://` and requires the GitHub App credentials.
