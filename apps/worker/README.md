...
Local development requires MongoDB, Qdrant, and Redis from `docker compose up -d`. With `AI_PROVIDER=google`, set `GOOGLE_API_KEY` (or `GEMINI_API_KEY`); no local model or OpenAI key is required. Private indexing and push-webhook processing additionally require `GITHUB_APP_ID` and `GITHUB_PRIVATE_KEY`. `INDEXING_CONCURRENCY` is bounded to 1-32 and `GITHUB_WEBHOOK_CONCURRENCY` to 1-8. Production requires TLS-managed MongoDB and Redis, HTTPS Qdrant with an API key, a configured AI provider, and the GitHub App credentials.
