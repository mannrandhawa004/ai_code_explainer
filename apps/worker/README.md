...
```text
clone -> scan/filter/hash -> changed-file chunks -> selected-provider embeddings -> selective Qdrant update -> MongoDB ready state
```
...
The worker reports structured progress, treats unsafe input, revoked private access, repository limits, exhausted hosted-provider credits, invalid provider credentials, and missing local models as unrecoverable, and supports cancellation between phases. `INDEXING_MAX_ATTEMPTS` controls transient retries and defaults to one for local learning; set it higher only when full-pipeline retries are desired. Completed indexing persists file imports/exports and normalized symbol/reference records in MongoDB.
...
Incremental change detection uses content hashes from a fresh safe clone. Existing metadata without per-file chunk counts triggers one complete rebuild. Otherwise only added or modified files reach Tree-sitter and the selected embedding provider. Deleted paths are removed, unchanged vector payloads are promoted to the new commit, and repository statistics and `lastIndexedCommit` are finalized only after synchronous Qdrant writes complete. Repository status is non-ready throughout the update, preventing partially updated search results from being served.
...
When `WORKER_METRICS_ENABLED=true`, the worker runs a small operational HTTP server on `WORKER_METRICS_HOST` and `WORKER_METRICS_PORT` (default `127.0.0.1:9464`). `GET /health` is the public container liveness endpoint and `GET /metrics` exposes bearer-protected Prometheus metrics for job outcomes and duration, active/stalled jobs, indexing volume, Qdrant calls, embeddings, and token usage. Production requires `METRICS_BEARER_TOKEN` to contain at least 32 characters. See the root [observability guide](../../docs/observability.md).

Local development requires MongoDB, Qdrant, and Redis from `docker compose up -d`. With `AI_PROVIDER=google`, set `GOOGLE_API_KEY` (or `GEMINI_API_KEY`); no local model or OpenAI key is required. For the 8 GB local Ollama profile, install `qwen3-embedding:0.6b` and `qwen2.5-coder:0.5b`, use a 1,024-dimension Qdrant collection, and keep `INDEXING_CONCURRENCY=1`. Compose routes backend requests to the Windows Ollama service through `OLLAMA_DOCKER_URL`. Private indexing and push-webhook processing additionally require `GITHUB_APP_ID` and `GITHUB_PRIVATE_KEY`. `INDEXING_CONCURRENCY` is bounded to 1-32 and `GITHUB_WEBHOOK_CONCURRENCY` to 1-8. Production requires TLS-managed MongoDB and Redis, HTTPS Qdrant with an API key, a configured AI provider, and the GitHub App credentials.
